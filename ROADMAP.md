...
These were findings from reading earlier code and records, not new benchmark results. The first four are addressed in comparison schema 3: fixture status is distinct from known synthetic corpus origin and claim scope; the three-paired-seed floor cannot be weakened; missing tokenizer artifacts block metric comparability; invalid/missing compute and metrics yield explicit ineligibility. Add regression tests if a later change reopens any of these paths. E11 now exports portable M2-GQA-002 evidence verified from a clean checkout; clean-environment reproduction and independent reruns remain. Every retroactive provenance correction must remain labeled as an audit rather than original capture. Never rewrite historical run files.

Implementation has since completed E01-E03, E06 and E07. E05 now has document-level and byte-aligned evaluation accounting; E08, E09 and E11 remain partial, with exact gates tracked in `docs/roadmap-measurement.md`. Q08 added isolated CPU inference-phase peaks, nested active/allocated sequence-state accounting, and cumulative sweep runtime caps across resumes. E08 still lacks an isolated CPU optimizer-step peak and CUDA hardware validation. Q09 portable evidence export is tracked and verified from a detached clean checkout; E11 still needs clean-environment training reproduction and independent reruns. The first new corpus is not frozen: the WikiText candidate has inconsistent license metadata, documented in `docs/data_protocol.md`. No research corpus download or sequence-architecture study has started under that unresolved selection.
