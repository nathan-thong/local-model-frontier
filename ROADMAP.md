...
Milestone 2 S01 now has GQA reference, gradient, cache-size and matched-compute config tests. A separate schema-3 TinyStories split records explicit synthetic origin and verified source/train/validation hashes; the historical schema-1 data remains unchanged. The MHA/GQA confirmation configs and three-seed model-free plan are frozen; no training run has started. The practical synthetic-domain loss budget is +0.02 nats/token. Next is a bounded stability smoke before the 5% screen or any confirmation run.
