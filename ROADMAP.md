...
Milestone 2 S01 now has GQA reference, gradient, cache-size and matched-compute config tests. A separate schema-3 TinyStories split now records explicit synthetic origin and verified source/train/validation hashes; the historical schema-1 data remains unchanged. No run has started. The practical synthetic-domain loss budget is +0.02 nats/token; MHA/GQA config copies and a dry-run plan must be frozen on a clean commit before the pilot.
