...
Milestone 2 S01 now has GQA reference, gradient, cache-size and matched-compute config tests. The synthetic TinyStories pilot is preregistered, but no run has started: the local schema-1 manifest must be replaced by a separately recorded schema-3 split, and the source must be committed cleanly before the pilot.
