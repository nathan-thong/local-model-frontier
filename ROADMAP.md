...
Implementation has since completed E01-E03, E06 and E07. E05 now has document-level and byte-aligned evaluation accounting; E08 and E09 remain partial, with exact gates tracked in `docs/roadmap-measurement.md`. Q08 added isolated CPU inference-phase peaks, nested active/allocated sequence-state accounting, cumulative sweep runtime caps across resumes, and sampled CPU optimizer-step readings. The follow-up step probe collected 11 readings during a 0.151018 s CPU step; the maximum is still sampled and can miss shorter transients. CUDA validation is unavailable on this host. Q09's portable evidence bundle verifies from a detached clean checkout; deterministic CPU fixture training reproduces in the documented environment and a separate clean clone on this host. This does not independently rerun the M2-GQA-002 comparison. Q10.a's OpenStax choice was withdrawn before acquisition after its publisher's LLM-training restriction was found. Q10-SOURCE-001 cleared zero of 24 Wikipedia revisions after the full-history and attribution review; no revision body was requested. Q10-SOURCE-002 screened 24 PMC candidates and passed 5; Q10-SOURCE-003 failed at query syntax. Q10-SOURCE-004 corrected the query grouping; 21 of 48 candidates passed metadata review, totaling 3,153,456 XML bytes. Q10-SOURCE-005 retrieved and verified all 21 authorized XML objects. SOURCE-006's explicit-href-only gate remains a historical failure: 13 permitted hrefs and eight ALI-only records left its minimum unattainable. The separate SOURCE-007 audit passed all 21 for restricted abstract/main-body prose under the frozen exclusions. Q10-PREP-002 has since prepared the restricted corpus twice with identical hashes: 21 articles, 593,275 UTF-8 bytes, 17/2/2 split, zero normalized duplicates, and an identical 512-token train-only byte-BPE artifact. The corpus result and held-out identities are in the [Q10-PREP-002 result inventory](docs/q10i_pmc_preparation_result.json); text, splits and tokenizer files remain ignored under data/. Q11.a and Q11.b have passed: the three-seed conventional GQA baseline completed on clean revision 549ff8d (mean validation 3.69769 bits/byte, sample SD 0.01007; task exact match 0/12). S02 now passes: explicit attention offsets and recursive state accounting reproduce same-checkpoint logits/cache bytes. S03 fixed-weight local-attention correctness and bounded-cache gates pass without training (see docs/s03_local_attention_result_v1.json); no sequence-module comparison has started. See [`docs/s02_state_result_v1.json`](docs/s02_state_result_v1.json).

Milestone 2 S01 has GQA reference, gradient, cache-size and matched-compute config tests. S02 now establishes explicit attention/recurrent state containers, absolute key offsets, recursive state traversal, module cost/state descriptors, and profile/state-accounting version identifiers. Same-checkpoint full/chunk/token logits and cache bytes agree before and after; the result is in [`docs/s02_state_result_v1.json`](docs/s02_state_result_v1.json). S03 fixed-weight local-attention correctness and bounded-cache gates pass without training (see docs/s03_local_attention_result_v1.json). A separate schema-3 TinyStories split records explicit synthetic origin and verified source/train/validation hashes; the historical schema-1 data remains unchanged. The MHA/GQA smoke and screen passed their plumbing gates, including exact 50% KV-cache reduction in the profiled smoke. The seed-17 screen NLL delta was -0.103 at -0.532% estimated compute; it is not claim-eligible. The first fresh three-seed full-budget attempt was ineligible because the schedules used different warmup fractions. M2-GQA-002 corrected that prospectively and passed the comparator on clean revision `73bde6e`: mean paired GQA-minus-MHA NLL delta was -0.02307 nats/token (SD 0.02439), and cache bytes were halved. CPU cached decoding was about 8.5% slower; this is a synthetic-domain quality/cache tradeoff, not a broad or statistically conclusive capability claim. See [the experiment record](EXPERIMENTS.md#m2-gqa-002---corrected-grouped-kv-confirmation-preregistered) and [the detailed execution plan](docs/post-confirmation-research-plan.md).
