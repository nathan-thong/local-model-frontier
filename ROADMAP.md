...
Implementation has since completed E01-E03, E06 and E07. E05 now has document-level and byte-aligned evaluation accounting; E08 and E09 remain partial, with exact gates tracked in `docs/roadmap-measurement.md`. Q08 added isolated CPU inference-phase peaks, nested active/allocated sequence-state accounting, cumulative sweep runtime caps across resumes, and sampled CPU optimizer-step readings. The follow-up step probe collected 11 readings during a 0.151018 s CPU step; the maximum is still sampled and can miss shorter transients. CUDA validation is unavailable on this host. Q09's portable evidence bundle verifies from a detached clean checkout; deterministic CPU fixture training reproduces in the documented environment and a separate clean clone on this host. This does not independently rerun the M2-GQA-002 comparison. Q10.a's OpenStax choice was withdrawn after the publisher page's LLM-training restriction was found, before corpus acquisition. Q10-SOURCE-001 preserved an English Wikipedia candidate inventory but cleared zero of 24 pages because full histories and attribution chains remained incomplete; no raw revision body was requested and no corpus text was saved. Q10-SOURCE-002 screened all 24 PMC candidates and passed 5; Q10-SOURCE-003 failed at query syntax. Q10-SOURCE-004 corrected the query grouping and passed all six translation checks. Of 48 metadata-screened candidates, 21 passed the version/date/license metadata gate (3,153,456 combined XML bytes; maximum 668,319 per object). Q10-SOURCE-005 then retrieved and verified all 21 authorized XML objects. Q10-SOURCE-006's href-only gate found 13 permitted hrefs and eight structured ALI license references without hrefs; its predeclared minimum could not be met, so body-level review stopped before clearance or extraction. Primary JATS/NISO documentation confirms `ali:license_ref` as an alternate URI pointer; Q10-SOURCE-007 is planned as a separate re-audit of the same 21 objects. No article prose or prepared corpus has been produced. The schema-4 train/validation/test splitter is ready. Complete the preregistered SOURCE-007 rights screen, then deterministic preparation and tokenizer/held-out freeze. Q11 task/scorer contracts, contamination checks and conventional baselines remain before any sequence-block training.
