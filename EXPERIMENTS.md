...
- **Status:** Planned only; blocked on a clean committed revision and a schema-3 TinyStories split. The local prepared directory currently has a schema-1 manifest with unknown origin and no current provenance hashes. No candidate run has started.
...
- **Change:** Derive `configs/m2/tinystories_v3_mha.json` and `configs/m2/tinystories_v3_gqa.json` from `configs/baseline_a.json` and `configs/gqa_a.json`, respectively, pointing both to the new schema-3 TinyStories split. The candidate changes KV heads from four to two. Match the new `decoder-module-mac-v1` estimated FLOP budget by increasing the candidate to 2,232,320 tokens (1,090 optimizer updates) from the MHA control's 2,097,152 tokens (1,024 updates). Keep the byte tokenizer, model width/depth, FFN, normalization, positional encoding, optimizer and evaluation split fixed.
...
- **Evaluation metrics:** Perplexity and NLL on the fixed validation split; byte-normalized bits-per-byte; analytical and actual KV storage at equal workloads; parameter/tensor/artifact bytes; training FLOPs, tokens and wall time; prefill/decode latency distributions; process and accelerator memory. The pilot is a stability/resource check only.
- **Result:** Pending; no training performed for this entry.
- **Interpretation:** Pending. No claim will be made from the single-seed pilot. Any qualifying multi-seed result is limited to the synthetic TinyStories domain and cannot establish human-authored or broad language capability.
- **Next experiment:** Reprepare the pinned 20 MiB prefix with its verified sidecar into a new schema-3 split, verify source/train/validation hashes, derive the two `configs/m2/` arms without changing the old baseline configs, and commit the experiment code/config before training. Then run the paired pilot through `frontier plan-sweep` and `frontier execute-sweep`. Proceed to three matched seeds only if the run is stable and an evaluation metric plus practical non-inferiority margin are frozen first.
