...
- **Status:** The seed-17 5%-compute screen is complete on clean revision `bad3aa4`; the fresh three-seed confirmation has not started. The MHA/GQA full-budget configs and initial three-seed dry-run plan are frozen at clean commit `4a8dd97`. The schema-3 split is local at `data/tinystories-v3`; its raw prefix, synthetic origin, source metadata, split hashes and manifest hash are recorded in `docs/data_protocol.md`. The M1 schema-1 data remains unchanged.
...
- **Training budget:** The seed-17 screen used five warmup updates and approximately 5% of the full per-seed budget: 51 MHA updates / 104,448 tokens / 677,905,956,864 FLOPs versus 54 GQA updates / 110,592 tokens / 674,296,233,984 FLOPs, a -0.53248% candidate delta. The planned confirmation uses three paired seeds; per seed MHA uses 2,097,152 tokens / 1,024 updates / 13,611,288,231,936 FLOPs and GQA uses 2,232,320 tokens / 1,090 updates / 13,610,794,352,640 FLOPs, a -0.00363% delta.
...
- **Interpretation:** The one-seed screen is directionally within the preregistered +0.02 nats/token practical budget, but its short schedule and one seed are not evidence of a capability improvement. It supports proceeding to the matched three-seed confirmation only. The smoke separately confirmed half-sized KV storage; the screen did not profile latency.
- **Next experiment:** From a clean revision, regenerate and verify the three-seed full-budget plan, then run all six paired jobs at 2,097,152 MHA tokens and 2,232,320 GQA tokens per seed. Compare only after every seed completes and profiles are collected; retain every failure and do not claim improvement unless the comparator gates pass.
