...
- **Status:** Planned only; infrastructure is frozen at commit `6ce18c5` and a local schema-3 split is prepared at `data/tinystories-v3`. The raw prefix, synthetic origin, source metadata, train/validation split hashes and manifest hash are recorded in `docs/data_protocol.md`. The M1 schema-1 data remains unchanged. No candidate run has started; MHA/GQA config copies and a dry-run plan remain prerequisites.
...
- **Next experiment:** Create and hash the two immutable config copies without changing the M1 configs; freeze a synthetic-domain validation metric and practical non-inferiority margin; generate and audit a dry-run plan. Commit that protocol/config revision before training. Then run the paired pilot through `frontier plan-sweep` and `frontier execute-sweep`. Proceed to three matched seeds only if the run is stable and the preregistered metric gate passes.
