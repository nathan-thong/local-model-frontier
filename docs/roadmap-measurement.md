...
| E11 - Portable evidence and CSV completeness | In progress | Nested workload profiles survive CSV export. Sanitized export, artifact/hash inventory, omission status, path scrubbing, comparison revalidation, and copied-bundle verification are complete for M2-GQA-002 (`results/m2-gqa-002/`). A clean Git checkout must still verify the tracked bundle; clean-environment training reproduction and independent reruns remain. |
...
Q09 implementation validation (23 September 2026): all 102 tests pass in 34.62 seconds, including rejection of unverified under-three-seed comparison eligibility; Ruff check and format pass across 49 files; local Markdown link targets resolve; `git diff --check` is clean; the wheel builds with SHA-256 `a4a788cacb08455974c58b701d78429f000cd2095a27f926a192ebbef7bdf89a`. CLI help is valid for export and verification. The six-run M2-GQA-002 export independently reruns `compare_seeded_runs` against the included source run artifacts and retains its eligible synthetic-domain comparison only when the reconstructed report matches. The portable bundle verifies in place and after copying to an isolated temporary directory; SHA-256 `df79b3578c70c6fa7c3a0d0a2d34e02f257cec16c14295287d73912d2b9bef3a`. It contains six sanitized configs and one sanitized data manifest; corpus text, tokenizer files, checkpoints and weights remain excluded and their availability/hashes are recorded. No absolute path was found. Verification from a clean Git checkout is the remaining Q09 gate.

Q09 closeout check (23 September 2026): all seven focused evidence tests pass, and `frontier verify-evidence --bundle-dir results/m2-gqa-002` reports `verified`, eight inventoried files, and the same bundle SHA-256. The bundle has not changed since full-suite validation. Q09 remains open only until the committed bundle verifies from a detached clean Git checkout.
