...
| E11 - Portable evidence and CSV completeness | In progress beyond minimum fixture contract | Nested workload profiles survive CSV export. Sanitized export, artifact/hash inventory, omission status, path scrubbing, comparison revalidation, and copied-bundle verification are complete for M2-GQA-002 (`results/m2-gqa-002/`). A documented Python environment ran the smallest deterministic CPU fixture twice from clean revision `74bbe3d28f3f80a00fac3a03aac0108036cf4cdd`; the second execution reproduced the model/checkpoint digest, manifest, and evaluation metrics. Q09-REPRO-002 will repeat once from a separate clean clone. This remains fixture-scope evidence, not an independent rerun of the M2-GQA-002 training comparison. |
...
Q09-REPRO-002 is preregistered at the next documentation checkpoint: one deterministic smoke execution from a separate clean checkout of `790a83ac5c60b26ed0bd983d267ae28374488bad`, with imports forced to the cloned source. Acceptance is fixture-only exact artifact/metric reproduction; this is not the independent M2-GQA-002 training rerun.
