...
| E11 - Portable evidence and CSV completeness | In progress | Nested workload profiles survive CSV export. Sanitized export, artifact/hash inventory, omission status, path scrubbing, comparison revalidation, and copied-bundle verification are complete for M2-GQA-002 (`results/m2-gqa-002/`). The tracked bundle verified from a detached clean checkout at `cc921b37ddcaf4ef9a223678fd2984b88647afbb`; clean-environment training reproduction and independent reruns remain. |

Latest Q08 validation (23 September 2026): all 95 tests pass in 28.48 seconds; the 31 focused memory/profiling/planning tests pass, including CPU worker-order isolation, shared backing storage, opaque state refusal, and graceful/interrupted cumulative-runtime resumes. Ruff check and format pass across 47 files; Markdown file targets resolve; `git diff --check` is clean; the wheel builds with SHA-256 `b03c3861a945b706558d50e7914761753a57423b158ee306566ef39230d22a2f`. The execute-sweep CLI help is valid. CUDA hardware behavior is unavailable on this host. Future status updates must include exact acceptance evidence; a code package can remain partial when its larger data or hardware gate is outstanding.

Q09 implementation validation (23 September 2026): all 102 tests pass in 34.62 seconds, including rejection of unverified under-three-seed comparison eligibility; Ruff check and format pass across 49 files; local Markdown link targets resolve; `git diff --check` is clean; the wheel builds with SHA-256 `a4a788cacb08455974c58b701d78429f000cd2095a27f926a192ebbef7bdf89a`. CLI help is valid for export and verification. The six-run M2-GQA-002 export independently reruns `compare_seeded_runs` against the included source run artifacts and retains its eligible synthetic-domain comparison only when the reconstructed report matches. The portable bundle verifies in place, after copying to an isolated temporary directory, and from a detached clean checkout; SHA-256 `df79b3578c70c6fa7c3a0d0a2d34e02f257cec16c14295287d73912d2b9bef3a`, eight inventoried files. It contains six sanitized configs and one sanitized data manifest; corpus text, tokenizer files, checkpoints and weights remain excluded and their availability/hashes are recorded. No absolute path was found. The clean-checkout verification used commit `cc921b37ddcaf4ef9a223678fd2984b88647afbb`; E11 remains in progress for clean-environment training reproduction and independent reruns.

Q09 closeout check (23 September 2026): all seven focused evidence tests pass, and `frontier verify-evidence --bundle-dir results/m2-gqa-002` reports `verified`, eight inventoried files, and the same bundle SHA-256. After commit `cc921b37ddcaf4ef9a223678fd2984b88647afbb`, the same command passed from a detached clean checkout with a clean worktree. Q09 is complete; broader E11 reproduction gates remain open.
