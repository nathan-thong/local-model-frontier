...
| E11 - Portable evidence and CSV completeness | Complete for the minimum reproduction contract | Nested workload profiles survive CSV export. Sanitized export, artifact/hash inventory, omission status, path scrubbing, comparison revalidation, and copied-bundle verification are complete for M2-GQA-002 (`results/m2-gqa-002/`). A new documented Python environment then ran the smallest deterministic CPU fixture twice from clean revision `74bbe3d28f3f80a00fac3a03aac0108036cf4cdd`; the second execution reproduced the model/checkpoint digest, manifest, and evaluation metrics. Scope is limited to same-environment fixture reproduction; this is not an independent rerun of the M2-GQA-002 training comparison. |
