...
| E04 - Freeze a useful control corpus | In progress; Q10.a complete, corrected | The OpenStax choice was withdrawn before acquisition after its publisher page's LLM-training restriction was found. The replacement is a bounded custom snapshot of English Wikipedia revisions, requiring exact oldid links, per-page CC BY-SA/attribution checks, and deterministic train/validation/test identities. WikiText lacks source-revision mapping; full PG-19 is outside the first local-preparation bound. No source text has been downloaded. |
...
| E08 - Memory accounting | In progress | Profile schema 4 measures prefill and cached-decode CUDA peaks separately and obtains CPU phase high-water marks in independent spawned workers. The profiler unions nested tensor-storage byte intervals once and reports allocated versus module-declared active state; opaque or non-contiguous cases remain explicitly unavailable. A separate command sampled current CPU worker RSS/private memory during one production optimizer step; the measured run had two samples over a gap about as long as the step, so transient interior peaks remain unresolved. Training summaries retain synchronized per-step CUDA peaks and absolute CPU snapshots. CUDA hardware behavior remains unvalidated on this host. |
