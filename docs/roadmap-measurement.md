...
| E08 - Memory accounting | In progress | Profile schema 4 measures prefill and cached-decode CUDA peaks separately and obtains CPU phase high-water marks in independent spawned workers. The profiler unions nested tensor-storage byte intervals once and reports allocated versus module-declared active state; opaque or non-contiguous cases remain explicitly unavailable. Training summaries retain synchronized per-step CUDA peaks and absolute CPU snapshots; a CPU optimizer-step high-water measurement remains open. CUDA hardware behavior is not validated on this host. |
