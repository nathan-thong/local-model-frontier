...
| Q11.b Baseline evaluation record | Luna | After Q11.a passes, preregister a bounded three-seed conventional GQA control on Q10 train data/tokenizer; run fixed configs and retain per-example/per-seed task outputs plus validation bits/byte. | **Preregistered:** fixed seeds 17/42/123, 131,072 tokens each, 256 updates, dense GQA config hashes and 2,164,663,517,184 estimated total training FLOPs are frozen in `docs/q11_baseline_prereg_v1.json`. Preflight passes, runs are not started. After validation, task and CPU resource evidence is recorded, sequence correctness work may begin. Keep Q10 test model metrics sealed. |

The schema-4 splitter retains legacy train/validation behavior and now supports frozen three-way identities and duplicate reports. The prepared Q10 corpus and tokenizer are local ignored artifacts with checksummed tracked metadata only. Q11.a task/scorer and contamination gates pass; Q11.b's bounded conventional baseline is preregistered and awaits three runs. The two test articles stay sealed for model metrics until their single declared final evaluation. See the [Q10-PREP-002 result inventory](q10i_pmc_preparation_result.json), [Q11 task-audit result](q11_task_contamination_result_v1.json), the versioned source inventories and `EXPERIMENTS.md`.
...
When work resumes, begin with the preregistered Q11.b baseline plan. Before running: inspect `git status`, read `AGENTS.md`, `RESEARCH.md`, `EXPERIMENTS.md`, `docs/measurement_protocol.md`, `docs/reproducibility.md`, `ROADMAP.md`, and the Q11 plan. Run all three seeds from the pinned clean revision, preserve failures and measured CPU resource evidence, and keep Q10 test model metrics sealed until the single final evaluation. Report Q08/Q09 limits and do not begin sequence-block training until the baseline and correctness gates pass.
