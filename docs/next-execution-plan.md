...
Freeze a bounded list of English Wikipedia titles and exact revision IDs before any text fetch. For each page, record the permanent revision link, source size, footer/history/talk review, license, attribution needs, and any imported or excluded content. If terms or attribution remain unresolved, exclude the page and replace it before acquiring the candidate set. Fetch serially with a contact-bearing User-Agent and `maxlag`; use deterministic extraction and explicit document identities. Keep corpus text out of Git. The Q10.b acceptance checks in the table above must pass twice before tokenizer fitting begins.
...
The generic schema-4 preparation path now writes deterministic train/validation/test files, hashed normalized document identities and a duplicate report; legacy train/validation manifests remain supported. This closes only the splitter implementation gate. The frozen 24-title inventory now pins exact revisions, and all candidates pass namespace, content-model and size checks (3,272,433 combined revision bytes). The first-pass rights review covered up to 500 recent history entries per page and current discussion material, but did not reach each page's full history; imported-content attribution and other source notices remain unresolved, so zero pages are cleared for acquisition. No raw revision text was requested through the API and no corpus text was saved. The body gate remains closed until at least 20 pages pass complete license and attribution review; see `docs/q10b_wikipedia_candidate_inventory.json` and `EXPERIMENTS.md`.
