...
The generic schema-4 preparation path now writes deterministic train/validation/test files, hashed normalized document identities and a duplicate report; legacy train/validation manifests remain supported. This closes only the splitter implementation gate. A 24-title candidate list, metadata-only query, and size/eligibility rules are preregistered in `docs/q10b_wikipedia_candidate_inventory.json`. Exact revision metadata, per-page rights review, deterministic extraction and two clean preparation runs remain open. No corpus text has been fetched.
