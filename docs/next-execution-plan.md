...
| Q11.b Baseline evaluation record | Luna | After Q11.a passes, preregister a bounded three-seed conventional GQA control on Q10 train data/tokenizer; run fixed configs and retain per-example/per-seed task outputs plus validation bits/byte. | **Next:** estimate and freeze a small three-seed plan plus maximum compute/runtime and failure handling before training. Keep test model metrics sealed until one final sequence-campaign evaluation after systems and analysis are frozen. |

The schema-4 splitter retains legacy train/validation behavior and now supports frozen three-way identities and duplicate reports. The prepared Q10 corpus and tokenizer are local ignored artifacts with checksummed tracked metadata only. Q11.a task/scorer and contamination gates pass; Q11.b's bounded conventional baseline is next. The two test articles stay sealed for model metrics until their single declared final evaluation. See the [Q10-PREP-002 result inventory](q10i_pmc_preparation_result.json), [Q11 task-audit result](q11_task_contamination_result_v1.json), the versioned source inventories and `EXPERIMENTS.md`.
