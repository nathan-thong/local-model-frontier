...
Q10.a has been amended twice before text acquisition. OpenStax *Principles of Economics 2e* was withdrawn because its publisher page prohibits LLM training without permission. Q10-SOURCE-001's 24-page Wikipedia screen cleared zero pages under the full-history/attribution standard. Q10-SOURCE-002 preregisters a metadata-only PMC Open Access search that freezes 24 PMCID candidates before screening per-version flags and exact CC BY 4.0 rights; it has not been run, and no article text has been acquired. The source decisions and rejected alternatives are recorded in `docs/data_protocol.md`.
