...
The earlier Wikipedia acquisition plan is historical: Q10-SOURCE-001 cleared none of its 24 candidates, so no page body was requested. The active source screen first freezes 24 PMC PMCID candidates, then checks per-version flags, exact CC BY 4.0 rights and XML size using metadata only. Do not request article XML until at least 20 candidates pass the rights/citation gate and their combined XML size is within 4 MiB. If the gate passes, fetch only eligible XML serially through an approved PMC service, inspect article-level rights and exclude material whose separate license cannot be verified. Then use deterministic extraction and document identities, keep corpus text out of Git, and require two clean preparations with matching source/content/split/tokenizer-input/manifest hashes before tokenizer fitting.

## Establish a useful evaluation domain before sequence training

Q10.a has been amended three times before text acquisition. OpenStax *Principles of Economics 2e* was withdrawn because its publisher page prohibits LLM training without permission. Q10-SOURCE-001's 24-page Wikipedia screen cleared zero pages under the full-history/attribution standard. Q10-SOURCE-002 froze 24 PMC PMCID candidates through six metadata-only searches; the first version screen found a 2017 article with CC BY 2.5, which failed. The exact-rights and date criteria were clarified before screening the remaining candidates. No article text has been acquired. The source decisions and rejected alternatives are recorded in `docs/data_protocol.md`.

| Step | Owner | Deliverable | Acceptance / stop rule |
|---|---|---|---|
| Q10.a Dataset decision | Luna | Compare candidate corpora using primary dataset cards, licenses, source revisions, scale, language/domain fit, and local preparation cost. Write each decision and amendment in `docs/data_protocol.md`. | **Complete, amended before acquisition.** OpenStax was withdrawn because its publisher page prohibits LLM training without permission. WikiText lacks article revision mapping; full PG-19 is outside the first local-preparation bound. The Wikipedia screen cleared 0/24 pages. Q10-SOURCE-002 preregisters a metadata-only PMC candidate screen for scientific prose. |
| Q10.b Deterministic preparation | Sol | Freeze 24 PMCID candidate records across six scientific strata by numeric query order, then screen PMC Cloud/OAI metadata without requesting full-text records. Require one unambiguous version per PMCID, at least one exact CC BY 4.0 OAI rights URI with no conflicting license, a consistent 2018–2025 publication year, complete citation metadata and the 1 MiB/article, 4 MiB combined caps. Request article XML only after at least 20 candidates pass. Then extract text, exclude unlicensed components, and write deterministic train/validation/test identities and duplicate reports. | Two clean preparations yield identical source/content hashes, split hashes, tokenizer inputs and manifest hash. Exclude separately licensed text or media unless its terms and attribution are preserved. No normalized duplicate crosses splits. Keep corpus text out of Git and stop if rights cannot be verified. |
