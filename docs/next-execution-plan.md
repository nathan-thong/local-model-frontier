...
| Q11.b Baseline evaluation record | Luna | After Q11.a passes, preregister a bounded three-seed conventional GQA control on Q10 train data/tokenizer; run fixed configs and retain per-example/per-seed task outputs plus validation bits/byte. | **Complete:** all three seeds consumed 131,072 tokens and 256 updates on revision `549ff8d`; no failures. Mean validation score was 3.69769 bits/byte (sample SD 0.01007), task exact match 0/12. Hash-only per-example and resource records are in `docs/q11_baseline_result_v1.json`; the sanitized portable bundle verifies at `367a82ff…`. Start sequence correctness S02; retain the Q10 test embargo for model metrics. |

The schema-4 splitter retains legacy train/validation behavior and now supports frozen three-way identities and duplicate reports. The prepared Q10 corpus and tokenizer are local ignored artifacts with checksummed tracked metadata only. Q11.a and Q11.b are complete; the baseline result and sanitized portable evidence are recorded in `docs/q11_baseline_result_v1.json` and `results/q11-baseline-001/`. The two test articles stay sealed for model metrics until their single declared final evaluation. Sequence correctness S02 is next, followed by the dense local-attention reference and one recurrent/linear reference. See the [Q10-PREP-002 result inventory](q10i_pmc_preparation_result.json), [Q11 task-audit result](q11_task_contamination_result_v1.json), [Q11 baseline result](q11_baseline_result_v1.json), the versioned source inventories and `EXPERIMENTS.md`.
...
When work resumes, start with sequence correctness S02 in `docs/roadmap-sequence.md`. Before editing: inspect `git status`, read `AGENTS.md`, `RESEARCH.md`, `EXPERIMENTS.md`, `docs/measurement_protocol.md`, `docs/reproducibility.md`, `ROADMAP.md`, and the sequence plan. Implement only explicit per-layer state, position offsets, accounting and parity checks; run the full validation suite. Keep Q10 test model metrics sealed, preserve all historical results, and report Q08/Q09 hardware and independent-rerun limitations. Do not begin sequence-module training until S02/S03 correctness passes and a fresh matched control/candidate plan meets the three-seed and compute gates.
