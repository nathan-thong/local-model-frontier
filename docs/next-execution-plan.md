...
| Q11.a Task-contract freeze | Luna | Select a small set of independently scored tasks with stable IDs, prompt/template version, scoring direction, generation limits, provenance and source/license; hand-check scorer examples and run deterministic contamination checks. Preserve byte-normalized likelihood where tokenizers differ. | Hand-calculated examples pass; evaluator is deterministic under declared settings; no task overlaps train, validation or sealed-test identities by normalized content. Keep all four held-out articles out of task selection. |
| Q11.b Baseline evaluation record | Luna | Define a bounded conventional dense baseline on the frozen data/tokenizer, record its compute budget, train fresh baseline seeds only after Q11.a passes, and write per-example/per-seed outputs. | Evaluation and failure records are complete; baseline/control uses the same revision, data, tokenizer and task contract. Keep any test evaluation to one declared final pass; do not start sequence-block training until the baseline artifact is frozen. |
