...
The generic schema-4 preparation path now writes deterministic train/validation/test files, hashed normalized document identities and a duplicate report; legacy train/validation manifests remain supported. This closes only the splitter implementation gate. The exact article/revision inventory, per-page rights review, deterministic extraction and two clean preparation runs remain open. No corpus text has been fetched.
