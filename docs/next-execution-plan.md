...
## Q09 bundle and minimum fixture reproduction: complete; independent clean-checkout rerun pending

The exporter, verifier, CLI, adversarial tests, sanitized six-run result bundle, and evidence documentation are committed in `cc921b37ddcaf4ef9a223678fd2984b88647afbb`. A detached clean checkout verified `results/m2-gqa-002/` with `frontier verify-evidence --bundle-dir results/m2-gqa-002`: status `verified`, bundle SHA-256 `df79b3578c70c6fa7c3a0d0a2d34e02f257cec16c14295287d73912d2b9bef3a`, eight inventoried files, and a clean worktree. The minimum deterministic fixture reproduction was run twice in the documented environment. A third run from a separately cloned clean checkout remains pending; no full M2-GQA-002 training rerun is planned in this bounded package.
...
| Q09.f Independent clean-checkout fixture rerun | Luna | From a separate local clone at pinned revision `790a83ac5c60b26ed0bd983d267ae28374488bad`, run `configs/smoke.json` once with a fresh output directory; compare checkpoint hash, fixture manifest, update/token counts and validation metrics with Q09.e. Use the established Python environment and force imports from the cloned source tree. | **Pending.** This is one 30-update / 7,680-token fixture rerun. Preserve any mismatch and investigate without changing the recipe. It is independent checkout evidence on the same host/environment, not an independent M2-GQA-002 comparison rerun. |
...
## Q09-REPRO-002 independent clean-checkout smoke: active bounded package

From a separate local clone of clean revision `790a83ac5c60b26ed0bd983d267ae28374488bad`, run `configs/smoke.json` once with a new output directory and Python imports forced to that clone's `src/`. Use the established Python 3.12.14 / PyTorch 2.14.0+cpu environment. Compare completion, clean source revision, fixture manifest, checkpoint hash, 7,680 tokens / 30 updates, train loss and validation metrics against Q09.e. Stop on any mismatch and preserve the run. This is a same-host fixture reproduction, not an M2-GQA-002 rerun; do not spend the full multi-seed comparison budget here.
...
| Step | Owner | Deliverable | Acceptance / stop rule |
|---|---|---|---|
| Q10.a Dataset decision | Luna | Compare candidate corpora using primary dataset cards, licenses, source revisions, scale, language/domain fit, and local preparation cost. Write the decision and rejected alternatives in `docs/data_protocol.md`. | **Complete, corrected before acquisition.** OpenStax was withdrawn because its publisher page prohibits LLM training without permission. The replacement is a bounded English Wikipedia snapshot by exact article revision, with page-level license and attribution audit. WikiText remains rejected because its release lacks the necessary source-revision mapping; PG-19's full corpus remains outside the first local-preparation bound. |
| Q10.b Deterministic preparation | Sol | Freeze a bounded list of English Wikipedia titles and exact revision IDs before fetching text; record revision permalinks, source sizes, and page-specific license/attribution checks. Fetch serially through the MediaWiki API with an identifying User-Agent and `maxlag`; implement deterministic text-only extraction, explicit train/validation/test outputs, document identities, and duplicate report. | Two clean preparations yield identical revision inventory, source/content hashes, split hashes, and manifest hash. Exclude pages with fair-use or unresolved imported-content notices. No normalized duplicate crosses splits. Keep corpus text out of Git and stop if terms or attribution cannot be verified. |
