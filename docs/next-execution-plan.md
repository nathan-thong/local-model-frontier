...
The earlier Wikipedia acquisition plan is historical: Q10-SOURCE-001 cleared none of its 24 candidates, so no page body was requested. Q10-SOURCE-002 (5/24) and Q10-SOURCE-003 (zero IDs after a query syntax failure) also remain closed historical attempts. Q10-SOURCE-004 passed the metadata gate: all six query translations retained the grouped exclusions, 48 unique PMCID candidates were screened, and 21 passed with 3,153,456 combined XML bytes (maximum 668,319 per object). Q10-SOURCE-005 retrieved only those 21 XML objects through current PMC Cloud; all passed frozen size and MD5 checks. No article prose has been extracted. Q10-SOURCE-006 froze an exact HTTP/HTTPS URI-scheme equivalence rule before cohort rights decisions, prompted by one license-element pilot whose href is HTTP while article license text and frozen OAI rights identify canonical HTTPS CC BY 4.0. The next package is the full rights, identity and third-party-text screen. Require at least 20 complete documents to remain before deterministic extraction and train/validation/test identity freezing. Keep text out of Git and require two clean preparations with matching source/content/split/tokenizer-input/manifest hashes before tokenizer fitting.

## Establish a useful evaluation domain before sequence training

Q10.a has undergone multiple source amendments before text acquisition. OpenStax *Principles of Economics 2e* was withdrawn because its publisher page prohibits LLM training without permission. Q10-SOURCE-001's 24-page Wikipedia screen cleared zero pages under the full-history/attribution standard. Q10-SOURCE-002 froze 24 PMCID candidates and passed 5 metadata records (547,447 XML bytes); Q10-SOURCE-003 returned zero IDs because ESearch treated NOT as a positive term. Q10-SOURCE-004 corrected the grouping and passed all six translation checks; 21 of 48 candidates passed metadata review (3,153,456 XML bytes). Q10-SOURCE-005 retrieved exactly those 21 XML objects with integrity checks passing. No article prose has been extracted, and no corpus or tokenizer has been frozen. Q10-SOURCE-006 froze a narrow license-URI scheme amendment after one pilot and before cohort rights review. Complete the rights and third-party-text screen first. The source decisions and rejected alternatives are recorded in `docs/data_protocol.md` and the versioned candidate inventories.

| Step | Owner | Deliverable | Acceptance / stop rule |
|---|---|---|---|
| Q10.a Dataset decision | Luna | Compare candidate corpora using primary dataset cards, licenses, source revisions, scale, language/domain fit, and local preparation cost. Write each decision and amendment in `docs/data_protocol.md`. | **Source selection in progress; metadata gate passed.** OpenStax was withdrawn because its publisher page prohibits LLM training. WikiText lacks article revision mapping; full PG-19 is outside the first local-preparation bound. The Wikipedia screen cleared 0/24 pages. Q10-SOURCE-002 passed 5/24 metadata records; Q10-SOURCE-003 returned zero IDs after a syntax failure. Q10-SOURCE-004 passed 21/48 records with 3,153,456 combined XML bytes. Article-level rights review is next; no corpus text has been acquired. |
...
The generic schema-4 preparation path now writes deterministic train/validation/test files, hashed normalized document identities and duplicate reports; legacy train/validation manifests remain supported. This closes only the splitter implementation gate. Q10-SOURCE-001 froze 24 exact English Wikipedia revisions with 3,272,433 combined bytes, but zero pages cleared full-history and attribution review. Q10-SOURCE-002 froze 24 PMCID candidates; 5 passed metadata review. Q10-SOURCE-003 failed at query syntax. Q10-SOURCE-004 froze and screened 48 candidates; 21 passed metadata checks, totaling 3,153,456 bytes. This clears only the metadata gate: fetch only these 21 records for article-level rights inspection, then require at least 20 complete eligible documents before preparation. See the versioned inventories and `EXPERIMENTS.md`.
