...
## Q09 closeout: complete

The exporter, verifier, CLI, adversarial tests, sanitized six-run result bundle, and evidence documentation are committed in `cc921b37ddcaf4ef9a223678fd2984b88647afbb`. A detached clean checkout verified `results/m2-gqa-002/` with `frontier verify-evidence --bundle-dir results/m2-gqa-002`: status `verified`, bundle SHA-256 `df79b3578c70c6fa7c3a0d0a2d34e02f257cec16c14295287d73912d2b9bef3a`, eight inventoried files, and a clean worktree. The full 102-test suite, Ruff, and diff checks pass. No corpus acquisition, architecture training, or new comparison was started as part of that closeout; Q09.e below adds only a bounded deterministic fixture reproduction.
...
Q10.a completed 23 September 2026. The selected source is the 2018 OpenStax *Principles of Economics 2e* textbook, using the publisher's versioned PDF (`PE2-2017-002-(03/18)-LC`) and its book-specific CC BY 4.0 statement. Candidate review and the WikiText rejection are recorded in `docs/data_protocol.md`. No source file was acquired; Q10.b begins with a hash and license-scope check before extraction.

| Step | Owner | Deliverable | Acceptance / stop rule |
|---|---|---|---|
| Q10.a Dataset decision | Luna | Compare candidate corpora using primary dataset cards, licenses, source revisions, scale, language/domain fit, and local preparation cost. Write the decision and rejected alternatives in `docs/data_protocol.md`. | **Complete.** The book-specific publisher page and PDF identify OpenStax *Principles of Economics 2e*, CC BY 4.0, revision `PE2-2017-002-(03/18)-LC`; WikiText is rejected over unresolved item-level license/attribution, and PG-19 over full-corpus size/domain. No data was acquired. |
| Q10.b Deterministic preparation | Sol | Add explicit test-split output to the preparer; implement only the selected OpenStax source adapter; pin the source PDF, extractor and preprocessing; document byte and memory bounds; create chapter-level train/validation/test identities and duplicate report. | Two clean preparations yield identical source, split and manifest hashes. The extracted text is within the book's CC BY 4.0 scope, no normalized duplicate crosses splits, and no uncontrolled endpoint or undocumented filtering is used. Do not commit corpus text. |
...
When work resumes, begin with Q10.a in `docs/implementation_queue.md`. Before editing: inspect `git status`, read `AGENTS.md`, `RESEARCH.md`, `EXPERIMENTS.md`, the measurement protocol, and the target specification. Implement one package only. Add tests that try to break the acceptance criteria, run focused tests then the repository-wide test/style/build/link checks, and update exact status evidence. Preserve historical runs. Do not train or acquire data before the corpus and evaluation gates pass. End with the commit/status, files changed, exact commands/results, unresolved limitations, and the next row.
