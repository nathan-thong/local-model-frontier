...
- **Gate:** Passed for fixed-weight semantics and bounded retained state. No prefill speed or FLOP-saving claim follows. S06's C0 correctness and CPU resource reference also pass; its toy dependency screen remains before architecture-comparison training. Defer S04 training until the toy gate passes; when S04 is considered, freeze its pilot plan and a fresh same-revision GQA control under Q10/Q11's frozen train/validation evaluation protocol.

## S04 — Test the local-attention capability tradeoff cheaply

- **Execution order:** Consider after the S06 C0 reference and toy dependency-learning gate have passed. This ordering is a research-sequence decision; S04 still tests the dense local block independently.

- **Prerequisites:** S03; the S06 C0 correctness/resource and toy dependency-learning gates; accounting that prices the actual dense implementation; frozen corpus and evaluation. For any comparison, train a fresh conventional GQA control and the local candidate at the same clean revision.
...
- **Current priority:** The C0 implementation, correctness and CPU resource reference are complete at `502014799903fc9500cebf109acc05eed8ba2e5c`; see [`s06_linear_reference_result_v1.json`](s06_linear_reference_result_v1.json). S06-DP-001 is preregistered; its generator, capped trainer, scorer and runner are implemented, and 177 tests plus Ruff checks pass. Execute it from a clean committed revision before any architecture-comparison training.
...
- **Experiment:** C0 correctness and CPU resource profiling are complete with zero training. S06-DP-001 is preregistered as a three-seed synthetic four-pair recall stability screen with fixed 256-update and 39B estimated-FLOP caps. Its runner records full-vocabulary and conditional retrieval accuracy by joint target-slot × distractor/distance stratum, loss, gradient norms, recurrent state norms, hashes, CPU process memory and elapsed time. Run it from a clean committed revision. Any later architecture comparison needs fresh same-revision conventional GQA control(s) under the frozen evaluation protocol.
