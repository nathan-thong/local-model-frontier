...
- **Current priority:** The C0 implementation, correctness and CPU resource reference are complete at `502014799903fc9500cebf109acc05eed8ba2e5c`; see [`s06_linear_reference_result_v1.json`](s06_linear_reference_result_v1.json). The next bounded package is to preregister and run the toy dependency-learning screen. Do not begin architecture-comparison training until that gate passes.
...
- **Experiment:** C0 correctness and CPU resource profiling are complete with zero training. The next package is a preregistered minimal dependency-learning fixture as a correctness/stability screen; record retrieval accuracy by distractor count/distance, loss, gradient norm, state norms, and time. Any subsequent one-seed pilot at equivalent disclosed compute against full GQA needs a preregistered budget and a fresh same-revision GQA control.
